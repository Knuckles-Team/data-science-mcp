#!/usr/bin/python
"""Scalar reward / value head over a transformer backbone (CONCEPT:ML-009).

A single linear head reading the backbone's last-layer hidden states powers both
the **reward model** (CONCEPT:ML-008 — one sequence-level score at the last real
token) and the **PPO value function** (CONCEPT:ML-009 — a per-token value
...
def attach_scalar_head(base_model: str, lora: Any | None = None) -> Any:
    ...
    Used by the reward-model trainer (CONCEPT:ML-008) and as the default PPO value
    function (CONCEPT:ML-009) when one is not dependency-injected.
    """
