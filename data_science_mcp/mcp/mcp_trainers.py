...
def _run_pretrain(kind: str, dataset: list, options: dict) -> dict[str, Any]:
    ...
    cfg_keys = {
        "base_model", "output_dir", "epochs", "lr", "batch_size", "max_steps",
        "max_seq_len", "grad_accum", "seed", "device", "precision",
        "gradient_checkpointing", "max_grad_norm", "warmup_steps", "lr_scheduler",
        "resume_from", "save_steps", "save_total_limit", "attn_impl", "use_liger",
        "pack_sequences", "distributed", "cpu_offload", "zero_stage", "tracker",
        "run_name", "kg_log",
    }
    spec_keys = {
